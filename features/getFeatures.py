...
def getFeatures(lines):
    features = np.array()  # full list of features for each line
    for i in range(len(lines)):  # for each line
        ...
        line_features = np.array([ # features for this line
            ...
            # top words placeholder, maybe not needed
        ...
        np.append(features,
                  line_features)  # add this line's features to full list
    return features
...
def createDataset(csv_filename, character):
    ...
    ones_feats = getFeatures(
        ones_lines)  # features for focus character's lines
    ...
    zeros_feats = getFeatures(
        zeros_lines)  # features for remainin characters' lines
    ...
    df.to_csv(
        csv_filename[:-4] + character + ".csv",
        index=False,
        encoding='utf-8-sig')  # save to .csv
    return df
